...
public class RobotContainer {
  ...
  public static TalonMotor m_simpleCorral;
  ...
  public RobotContainer()
   {
    ...
    sPID = new ShooterPID();
    // m_simpleCorral = new TalonMotor(4, false);
    mongClimb = new Climb();
  ...
  private void configureButtonBindings() 
  {
    ...
    y.toggleWhenPressed(new HarvesterUpDown());
     // releasing the harvester down or up
   ...
   // eleven.whileHeld(new RevCor(true)); // sending the balls to the shooter 
    ...
    nine.whileHeld(new ClimbNow(false, 80, false, 0.5)); // unratchet deploy then deploy motor for a little bit then retract motor
    nine.whenReleased(() -> {ClimbNow.amount = 1;});
    ...
    // ten.whileHeld(new SpinTalonMotor(m_simpleCorral, 0.5));
    // twelve.whileHeld(new SpinTalonMotor(m_simpleCorral, -0.5));
    trigger.whileHeld(new ShootAndCorral(10)); // shooter at 90 rn
    a.toggleWhenPressed(new ToggleHarvest(1));
    ex.toggleWhenPressed(new ToggleHarvest(2));
  ...
  public static boolean getLeftTrigger() {
    ...
    {
      held = !(xbox.getRawAxis(2)>0.5);
      return false;
    }
  }

  static boolean heldTwo = false;
